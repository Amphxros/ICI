...
import es.ucm.fdi.ici.c2122.practica5.utils.simFunctions.AbsSim;
import es.ucm.fdi.ici.c2122.practica5.utils.simFunctions.AverageMoveSim;
import es.ucm.fdi.ici.c2122.practica5.utils.simFunctions.CosineSim;
import es.ucm.fdi.ici.c2122.practica5.utils.simFunctions.MapSim;
import es.ucm.fdi.ici.c2122.practica5.utils.simFunctions.MoveSim;
...
public class MsPacManCBRengine implements StandardCBRApplication {
	...
	private MOVE action;
	private MsPacManStorageManager storageManagerGeneric;
	...
	public MsPacManCBRengine(MsPacManStorageManager storageManagerGeneric, MsPacManStorageManager storageManagerSpecific, Vector<Double> w){
	...
	private MOVE reuse(Collection<RetrievalResult> evalGeneric, Collection<RetrievalResult> evalSpecific)
	{
		//Aqui tenemos que consultar el contador para elegir los casos de las bases que van al proceso de seleccion
				...
				par = this.Ponderada(genericRetrival, specificRetrival);
				...
				par = this.roulete(genericRetrival, specificRetrival);
...
private Pair<MOVE , Double> MaxSim(Collection<RetrievalResult> genericRetrival, Collection<RetrievalResult> specificRetrival) {
		...
		MsPacManResult result = new MsPacManResult();
		...
		for(RetrievalResult r: genericRetrival) {
			if((Math.abs(r.getEval() - similarity) < 0.5) && (similarity != 0)) {
				MsPacManResult result2 = (MsPacManResult) r.get_case().getResult();
				...
				similarity = r.getEval();
				result = (MsPacManResult) r.get_case().getResult();
				solution = (MsPacManSolution) r.get_case().getSolution();
				toAddCase = "generic";
			}
		}
	...
	private Pair<MOVE , Double> roulete(Collection<RetrievalResult> genericRetrival, Collection<RetrievalResult> specificRetrival) {
		...
		for(CaseInfo c: list) {
			if(rand < c.getAcumulada()) {
				...
				solution = (MsPacManSolution) c.getCaso().getSolution();
