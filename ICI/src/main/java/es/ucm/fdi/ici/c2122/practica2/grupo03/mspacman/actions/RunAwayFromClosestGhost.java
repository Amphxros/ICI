...
public class RunAwayFromClosestGhost implements Action {
	...
	public MOVE execute(Game game) {
		...
		if(!game.isGhostEdible(closestGhost)) {
			...
			return game.getApproximateNextMoveAwayFromTarget(pacmanNode, game.getGhostCurrentNodeIndex(secondGhost), game.getPacmanLastMoveMade(),DM.PATH);
		}
		else if(!game.isGhostEdible(thirdGhost)) {
			return game.getNextMoveAwayFromTarget(pacmanNode, game.getGhostCurrentNodeIndex(thirdGhost), DM.PATH);
			
		}
		else {
			return game.getNextMoveAwayFromTarget(pacmanNode, game.getGhostCurrentNodeIndex(farthestGhost), DM.PATH);
